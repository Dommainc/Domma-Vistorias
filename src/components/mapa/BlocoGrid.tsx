...
import { STATUS_BG, STATUS_LABEL, VISTORIA_STATUS_LABEL } from '@/types/mapa'
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 xl:grid-cols-10 gap-1 p-2">
        {unidadesVisiveis.map((u, i) => {
          ...
          return (
            ...
            >
              {/* Conteúdo central */}
              <div className="flex flex-col items-center justify-center h-full px-1 pt-1.5 pb-5 gap-0.5">
                ...
                {(u.area_privativa ?? u.area_total) != null && (
                  <span className="text-[9px] opacity-85 leading-none">
                    {(u.area_privativa ?? u.area_total)!.toFixed(2)} m²
                  </span>
                )}
              </div>
              ...
              <div
                className="absolute bottom-0 left-0 right-0 text-center text-[9px] font-semibold uppercase leading-none py-1 tracking-wide"
