...
serve(async (req) => {
  ...
  try {
    ...
    const idPessoa = url.searchParams.get('id')
    if (!idPessoa) {
    ...
    const response = await fetch(
      `${CVCRM_BASE_URL}/cvbot/pessoa?id_pessoa=${idPessoa}`,
      {
        headers: {
    ...
    if (!response.ok) {
      ...
      return new Response(
        JSON.stringify({ error: `Erro ao buscar pessoa CVCRM: ${response.status}`, detail: text }),
